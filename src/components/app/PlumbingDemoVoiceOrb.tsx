...
const PLUMBING_WIDGET_ID = "69b161140ec30015b844b0d2";
const PLUMBING_HOST_ID = "ghl-plumbing-example-widget-inline";
...
export function PlumbingDemoVoiceOrb({ includeScript = true }: { includeScript?: boolean }) {
    ...
    const getWidgetHost = useCallback(() => {
        const selector = [
            `chat-widget[widget-id="${PLUMBING_WIDGET_ID}"]`,
            `chat-widget[data-widget-id="${PLUMBING_WIDGET_ID}"]`,
            `chat-widget#${PLUMBING_HOST_ID}`,
        ].join(", ");

        return document.querySelector(selector) as HTMLElement | null;
    }, []);

    const getWidgetRoot = useCallback(() => getWidgetHost()?.shadowRoot ?? null, [getWidgetHost]);
    ...
    const applyWidgetStyles = useCallback(() => {
        ...
        const styleId = `${PLUMBING_HOST_ID}-voice-style`;
    ...
    const syncCallState = useCallback(() => {
        ...
        const callStatus =
            root.querySelector(".lc_text-widget--voice-call-status")?.textContent?.trim().toLowerCase() ?? "";
        ...
        <>
            ...
            <div className="flex w-full flex-col items-center">
                <div className="relative">
                    <motion.div
                        aria-hidden="true"
                        className={`absolute inset-[-18px] rounded-full blur-2xl ${isActive ? "bg-red-400/24" : "bg-teal-300/26"}`}
                        animate={
                            isActive
                    ...
                    {!isActive ? (
                        <motion.div
                            aria-hidden="true"
                            className="absolute inset-[-8px] rounded-full blur-md bg-[conic-gradient(from_0deg,rgba(255,255,255,0)_0deg,rgba(153,246,228,0.18)_65deg,rgba(45,212,191,0.18)_145deg,rgba(56,189,248,0.14)_225deg,rgba(255,255,255,0)_320deg)]"
                    ...
                    >
                        {!isActive ? (
                            <>
                                <motion.div
                                    aria-hidden="true"
                                    className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_28%_24%,rgba(255,255,255,0.3),transparent_18%),radial-gradient(circle_at_74%_72%,rgba(45,212,191,0.2),transparent_24%)]"
                                ...
                                <motion.div
                                    aria-hidden="true"
                                    className="absolute inset-[8px] rounded-full border border-teal-100/18"
                                    animate={{ rotate: [0, -360] }}
                        ...
                        <div className="relative flex flex-col items-center gap-3 px-5 text-center">
                            <div
                                className={`flex h-11 w-11 items-center justify-center rounded-full border ${
                                    isActive ? "border-red-300/40 bg-red-500/10 text-red-100" : "border-teal-100/55 bg-teal-300/10 text-teal-100"
                                }`}
                            >
